...
export default function Home() {
  ...
  // 2. 🔥 แยกฟังก์ชันโหลดวันที่ออกมา เพื่อให้เรียกใช้ซ้ำได้
  const fetchDatesData = async () => {
    try {
      ...
      if (error) throw error;
      setAvailableDates(data.map(d => d.date));
      return true; // ส่งค่ากลับว่าเสร็จแล้ว
    } catch (err) {
      console.error("Failed to load dates:", err);
      return false;
    }
  ...
  useEffect(() => {
    ...
    // Realtime Listener
    const channel = supabase
      .channel('realtime-days')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'days' },
        () => {
          console.log('มีการเปลี่ยนแปลงวันเปิดจอง!');
          fetchDatesData(); 
        }
      )
      .subscribe();
  ...
  // --- 2. Load Slots ---
  const fetchSlotsData = async (selectedDate) => {
      if (!selectedDate) {
        setSlotStatus({ text: "", type: "" });
        setSlots([]);
        return;
  ...
  useEffect(() => {
    ...
    const bookingChannel = supabase
      .channel('realtime-bookings')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bookings' }, () => fetchSlotsData(date))
      .subscribe();

    const slotChannel = supabase
      .channel('realtime-slots')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'slots' }, () => fetchSlotsData(date))
      .subscribe();
  ...
  // --- 3. Handle Refresh (ฟังก์ชันสำหรับ PullToRefresh) ---
  const handleRefresh = async () => {
    // โหลดวันที่ใหม่
    await fetchDatesData();
    // ถ้ามีการเลือกวันอยู่แล้ว ให้โหลดสล็อตใหม่ด้วย
    if (date) {
        await fetchSlotsData(date);
    }
    return true; // บอก Library ว่าเสร็จแล้ว
  };
  ...
  return (
    <div className="min-h-screen flex font-sans bg-stone-50 relative">
      <style>{`
                ...
                /* ปรับแต่งตัว Pull To Refresh */
                .ptr__pull-down--pull-more { text-align: center; padding-top: 10px; color: #059669; }
                .ptr__loader { color: #059669 !important; }
            `}</style>
      ...
      {/* Right Side: Form */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-0 md:p-12 h-screen overflow-hidden bg-stone-50">
        {/* 3. 🔥 เอา PullToRefresh มาคร่อมเฉพาะส่วน Form นี้ */}
        <PullToRefresh 
        ...
        >
            <div className="w-full min-h-full p-4 sm:p-8 md:p-0">
                <div className="w-full max-w-md mx-auto space-y-8 fade-in-up pb-20">
